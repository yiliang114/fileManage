  ...
  render() {
      ...
      if (itemData.children && itemData.children.length) {
        itemData
          ...
          .forEach(menu => {
            if (menu.url === hash) {
              currentItem = menu;
              breadcrumbItems.push({name: itemData.name, url: itemData.url});

              breadcrumbItems.push({name: menu.name, url: menu.url});
            } else {
                ...
                menu
                  ...
                  .forEach(subpage => {
                    if (subpage.url === hash) {
                      ...
                      breadcrumbItems.push({name: subpage.name, url: subpage.url});
                    }
                  });
              }
            }
    ...
    return (
      <Breadcrumb className="bread">
        {breadcrumbItems.map((item, index) => <Breadcrumb.Item key={item.url}>{breadItemRender(item, index)}</Breadcrumb.Item>)}
      </Breadcrumb>
    );
  }
}
